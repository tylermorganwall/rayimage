...
plot_image_grid = function(
  ...
  gl = grid::grid.layout(nrow = dim[1], ncol = dim[2])
  layout_idx = data.frame(
    y = rep(seq_len(dim[1]), each = dim[2]),
    x = rep(seq_len(dim[2]), dim[1])
  )
  grid::pushViewport(grid::viewport(name = "image_array", layout = gl, gp = gp))
  for (i in seq_len(length(input_list))) {
    ...
    grid::pushViewport(
      viewport = grid::viewport(
        ...
        name = sprintf("grid_%i_%i", layout_idx$x[i], layout_idx$y[i]),
        gp = gp
      )
    )

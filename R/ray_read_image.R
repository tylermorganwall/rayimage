...
#'`JPEG`, `PNG`, `TIFF`, `TGA`, `BMP`, `PSD`, `GIF`, `HDR`, `PIC`, `PNM`, and `EXR` images.
#'
...
#' floating-point HDR formats (`EXR`, `HDR`); `FALSE` for all other file-based formats
...
ray_read_image = function(
  ...
  if (is.na(source_linear)) {
    ...
    source_linear = imagetype %in% c("array", "matrix", "exr", "hdr")
  }

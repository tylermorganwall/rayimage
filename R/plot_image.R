...
plot_image = function(input, rotate=0, keep_user_par = FALSE, ...) {
  ...
  if(length(dim(input)) == 3) {
    ...
    suppressWarnings(raster::plotRGB(raster::brick(input, xmn = 0.5, xmx = dim(input)[2]+ 0.5,ymn = 0.5, ymx = dim(input)[1] + 0.5, ...),scale=1, maxpixels=nrow(input)*ncol(input),...))
    ...
    suppressWarnings(raster::plotRGB(raster::brick(array_from_mat, xmn = 0.5, xmx = dim(array_from_mat)[2] + 0.5,ymn =  0.5, ymx = dim(array_from_mat)[1] +  0.5, ...),scale=1, maxpixels=nrow(input)*ncol(input), ...))
  } else {
